  ...
  <tbody>
    {providers.map((provider,index)=>(
        <tr>
            ...
            <td>
            <a class="btn btn-outline-primary" aria-current="page" href={`/profilep/${provider.id}`}>View</a>{' '} 
                <a class="btn bg-primary" aria-current="page" href={`/update/userp/${provider.id}`}>Update</a>{' '} 
                <a class="btn bg-danger" aria-current="page" onClick={()=>deleteProvider(provider.id)} href='loginp'>Delete</a>
